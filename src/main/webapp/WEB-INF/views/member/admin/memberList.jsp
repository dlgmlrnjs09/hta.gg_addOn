...
<div style="margin: 70px;">
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination">
			<c:if test="${pu.startPageNum>5 }">
				<li class="page-item">
					<a class="page-link" href="/member/admin/memberList?pageNum=${pu.pageNum-1}&field=${field}&keyword=${keyword}" aria-label="Previous">
			...
			<c:forEach var="i" begin="${pu.startPageNum}" end="${pu.endPageNum }">
				<c:choose>
					<c:when test="${i==pu.pageNum }">
						<li class="page-item"><a class="page-link" href="/member/admin/memberList?pageNum=${i}&field=${field}&keyword=${keyword}"><span style='color:blue;'>[${i }]</span></a></li>
					</c:when>
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="/member/admin/memberList?pageNum=${i}&field=${field}&keyword=${keyword}"><span style='color:gray;'>[${i }]</span></a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${pu.endPageNum<pu.totalPageCount }">
				<li class="page-item">
      				<a class="page-link" href="/member/admin/memberList?pageNum=${pu.pageNum+1}&field=${field}&keyword=${keyword}" aria-label="Next">
	...
	<div>
		<form:form method="post" action="/member/admin/memberList" class="form-inline" role="form">
