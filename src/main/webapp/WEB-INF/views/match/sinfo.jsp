...
<div id="listWrapper">
	<div id="mostList">
		<c:forEach items="${most }" var="item">
			...
			<div class="mostItem">
				<div class="mostFace">
					<a href="/champ/selectList?championid=${item.championid }">
		...
		<br>
		<hr>
		<p>같은 팀으로 플레이한 소환사별 승률</p>
		<c:forEach var="i" items="${duoRateList}">
			<div class="mostItem">
				<table id="duoRateTable">
					<th>${i.snickname}</th>
					<td><fmt:formatNumber value="${(i.winCount / i.count) * 100}" pattern=".00"/>%</td>
					<td>${i.count}전</td>
					<td>${i.winCount}승</td>
					<td>${i.loseCount}패</td>
				</table>
...
function getMatchList(args) {
	...
	isRunning = true;
	$.ajax({
		url:'/match/getList/${svo.snickname }/'+args,
		dataType:'json',
...
// 		console.log(sum);
		...
		let face =$("<div>",{class:'face',
			html:"<a href='/champ/selectList?championid="+sum.championid+"'>"+
...
function addMost3(mostList){
	...
	for(most of mostList){
		...
		let most3Face = $("<div>",{
					class:'most3Face',
					html:"<a href='/champ/selectList?championid="+most.championid+"'>"+
