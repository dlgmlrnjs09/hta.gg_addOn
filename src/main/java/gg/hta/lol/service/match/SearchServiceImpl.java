...
public class SearchServiceImpl implements SearchService {
	...
	public SearchVo getSummoner(String name) {
		String sid;
		String aid;
		JsonObject summonerInfo = dataRequester.getSummonerInfo(name); // api에서 소환사 정보 Json으로 받아옴
//		System.out.println(summonerInfo);
		if(summonerInfo==null) {
			return null;
		}
		
		sid = summonerInfo.get("id").getAsString(); // 받아온 Json객체에서 id값 추출
		aid = summonerInfo.get("accountId").getAsString(); // 받아온 Json객체에서 accountID 추출
		// 받아온 값으로 vo 생성 후 db에 insert or update
...
//				System.out.println("소환사 정보 수정");
	...
	// queueInfo 테이블 insert or update
	@Override
	public void addQueueInfo(String sid) {
		JsonArray leagueInfo = dataRequester.getLeagueInfo(sid); // 소환사 큐타입별 정보 받아옴
...
//				System.out.println("큐별 티어 정보 중복 정보 수정");
	...
	public void readMatchList(String aid,int start,int end,String snickname) {
		...
		JsonObject matchInfo = dataRequester.getMatchList(aid,0	,20); // 최근 20경기 정보 받아오기
//		JsonObject matchInfo = dataRequester.getMatchList(aid);
		
		JsonArray matchArr = matchInfo.get("matches").getAsJsonArray(); // 최근 20경기 정보 배열로 변환
...
//		summonerMapper.addSummonerList(sList);
	...
	@Override
	public List<HashMap<String, String>> getTargetMatchList(String snickname) {
		return matchinfoMapper.getTargetMatchList(snickname);
	}
